...
传输参数，把本地文件、有界 HTTP、live HTTP 或 HLS 音频统一解码为 48 kHz stereo PCM，
...
file / URL / live / HLS ───> decode → resample → DSP → Opus ───> OpusQueue ───> RTP/RTCP
```
...
- status 暴露 sender 的有界 playout diagnostics，宿主可批量观察 queue、underrun、lateness、drop和
  RTP进度，不依赖 Rust进程内 metrics recorder。
...
- [CPU/NUMA/I/O 部署调优](docs/performance-deployment.md)：CPU余量、NUMA分片、指令集和io_uring准入。
