...
HLS 是独立的第四条获取路径，但不把传输方式等同于直播语义。`.m3u8` URL 或
`formatHint: m3u8` 会先规范化为 `seekable: false`，runtime 解析最终 media playlist 后再分类：带
`EXT-X-ENDLIST` 的 VOD 保持 `kind: url`，没有结束标记的 playlist 才升级为 `kind: live`。playlist
和 segment 由 async 任务顺序获取，解复用后的音频进入同一个 bounded byte bridge；当前实现仍只允许
HLS 作为 current，也没有 seek、next preload 或隐式 timeshift。HLS VOD 可以暂停，HLS live 不可以。
