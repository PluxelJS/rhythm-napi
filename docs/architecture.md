...
HLS 是独立的第四条获取路径，但当前采用 live 语义而不增加新的公开 kind：`.m3u8` URL 或
`formatHint: m3u8` 会自动规范化为 `kind: live`、`seekable: false`。playlist 和 segment 由 async
任务顺序获取，解复用后的音频仍进入同一个 bounded live bridge；因此 HLS 同样只允许 current，
没有 pause、seek、next preload 或隐式 timeshift。
