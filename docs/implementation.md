...
`.m3u8` path 或显式 `formatHint: m3u8` 在进入 actor 前只规范化为 non-seekable；最终 media
playlist 带 `EXT-X-ENDLIST` 时保持 bounded `url`，否则 producer 通过 generation-scoped event 把
current 升级为 `live`。HLS source
...
pipeline 消费完 chunk 后把 PCM allocation 归还 decoder；Symphonia 在合理大小内复用该 allocation，
异常大的 packet 不长期滞留。Rubato 输出 buffer直接转移给下游并在消费后回收，复用池有固定条目
上限，不能用减少 allocation 为由制造与输入峰值等大的常驻内存。
...
降档配置。bitrate遵守已协商transport，N-API未显式提供时使用音乐默认128 kbps；不能超过接收端
协商能力来追求无效的名义高码率。payload上限来自`MTU - 12-byte RTP header`，编码结果直接进入
immutable `Bytes` frame。不要为“异步化”把编码搬到Tokio worker，也不要在producer与sender之间
增加另一个channel。
