...
- 你对 GitHub 仓库 `PluxelJS/rhythm-napi` 拥有 environment、secret 和 Actions 权限。
...
gh auth login
gh auth status
gh api --method PUT repos/PluxelJS/rhythm-napi/environments/npm
```
...
```sh
gh secret set NPM_TOKEN --env npm --repo PluxelJS/rhythm-napi
```
...
RUN_ID=$(gh run list --repo PluxelJS/rhythm-napi --workflow release.yml \
  --limit 1 --json databaseId --jq '.[0].databaseId')
gh run watch "$RUN_ID" --repo PluxelJS/rhythm-napi --exit-status
```

成功后，npm 上会出现主包和四个平台包。分别打开这五个包的 trusted publisher
设置，并全部填写同一组绑定：

- organization/user：`PluxelJS`
...
```sh
gh secret delete NPM_TOKEN --env npm --repo PluxelJS/rhythm-napi
```
