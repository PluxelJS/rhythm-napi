...
相互继承。已缓存 artifact 也必须符合当前 `maxBytes` 才会命中。
...
幂等策略；若按stream过滤drain，sequence间隔也可能来自其他stream或快照合并：

- `nextNeeded`：从 playlist 选择下一首并设置 next；没有下一首可以保持 idle。
- `sourceRefreshNeeded`：重新向 provider 获取同一内容 ID 的 URL，再 refresh current source。
