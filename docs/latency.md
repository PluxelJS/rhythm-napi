...
这是保守的容器能力门槛。扩展名判断会忽略 query和fragment；没有扩展名的签名URL可由宿主提供
规范化 `formatHint`。M4A/MP4通常依赖随机访问metadata，仍未进入渐进allowlist，会先下载完整
artifact，再使用seekable file decoder。
...
显式format hint已解决无扩展名URL退化；M4A/MP4仍需单独研究。只有metadata布局允许顺序probe，
或实现经过容器验证的索引/Range策略时才应进入渐进路径。
