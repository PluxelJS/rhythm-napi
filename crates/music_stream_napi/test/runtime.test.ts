...
test('status queries enforce bounded validated stream identifiers', async () => {
	...
	try {
		...
		await expect(
			streamer.getStatuses(Array.from({ length: 4_097 }, (_, index) => `stream-${index}`)),
	...
	try {
		...
		await expect(
			streamer.startStream({
				...
				current: { id: 'overflow', attemptId: 'attempt-overflow', kind: 'file', path: firstPath },
		...
		await expect(
			streamer.startStream({
				...
				current: { id: 'closed', attemptId: 'attempt-closed', kind: 'file', path: firstPath },
...
test('seek near the end preserves repeated automatic preload promotion', async () => {
	...
	try {
		...
		await waitForStatus(
			...
			throw new Error(
				`b did not promote after seek: ${JSON.stringify(await streamer.getStatus(streamId))}`,
				{
					cause: error,
				},
			)
		...
		const final = await waitForStatus(
			...
			throw new Error(
				`c did not promote after b: ${JSON.stringify(await streamer.getStatus(streamId))}`,
				{
					cause: error,
				},
			)
...
test('immediate seek to the end releases promoted HTTP preload quota for the following track', async () => {
	...
	try {
		await streamer.startStream({
			streamId,
			current: { id: 'a', attemptId: 'attempt-a', kind: 'file', path: currentPath },
		...
		await waitForStatus(
			...
			throw new Error(
				`delayed b did not promote: ${JSON.stringify(await streamer.getStatus(streamId))}`,
				{
					cause: error,
				},
			)
...
test('failed next preload after current end exits buffering and requests a replacement', async () => {
	...
	try {
		...
		await streamer.reconcilePlan(streamId, {
			...
			next: { id: 'following', attemptId: 'attempt-following', kind: 'file', path: followingPath },
		})
		await waitForStatus(
			() => streamer.getStatus(streamId),
	...
	try {
		...
		await streamer.startStream({
			...
			current: {
				...
				attemptId: 'attempt-misclassified-radio',
				kind: 'url',
				url: server.url,
				formatHint: 'wav',
				seekable: true,
...
test('opaque HLS VOD is detected without being mislabeled as live', async () => {
	...
	try {
		...
		await streamer.startStream({
			...
			current: {
				id: 'opaque-hls-radio',
				attemptId: 'attempt-opaque-hls-radio',
				kind: 'url',
				url: server.url,
				seekable: true,
			},
...
test('HLS without an end list is classified as live after probing the media playlist', async () => {
	...
	try {
		...
		const started = await streamer.startStream({
			...
			current: {
				id: 'live-hls-radio',
				attemptId: 'attempt-live-hls-radio',
				kind: 'url',
				url: server.url,
				seekable: true,
			},
